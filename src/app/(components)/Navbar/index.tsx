  ...
  return (
        <div className="flex justify-between bg-white px-4 py-3 dark:bg-dark dark:px-4 dark:py-3">
          ...
          <div className="flex items-center">
            <button onClick={()=>dispatch(setIsDarkMode(!isDarkMode))} className={isDarkMode ? `rounded p-2 dark:hover:bg-gray-700` : `rounded p-2 hover:ring-gray-100`}>
              {isDarkMode ? <Sun className="h-6 w-6 cursor pointer dark:text-white"/> : <Moon className="h-6 w-6 cursor pointer dark:text-white"/>}
            </button>
            <Link href="/settings" className={isDarkMode ? `h-min w-min rounded p-2 dark:hover:bg-gray-700` : `h-min w-min rounded p-2 hover:ring-gray-100`}>
              <Settings className="h-6 w-6 cursor pointer dark:text-white"/>
            </Link>
            <div className="ml-mr-5 hidden min-h-[2em] w-[0.1rem] bg-gray-200 md:inline-block">
